...
tool_choice = st.sidebar.radio("اختر الأداة:", ["حاسبة التجارة الإلكترونية", "حاسبة أرباح التداول", "القوالب الجاهزة (مجاناً)"])
...
# ==========================================
    ...
    st.write("ملف إكسيل جاهز لتسجيل فواتير المشتريات وحساب تكلفة الوحدة للمنتجات المستوردة وتوزيع مصاريف الشحن بدقة.")
    st.download_button(label="📥 تحميل قالب الموردين", data="هذا مجرد ملف تجريبي لتعمل الأداة", file_name="suppliers_ledger_template.csv")
    
    st.subheader("2. نموذج جرد المخزون الدوري (PDF)")
    st.write("جدول مبسط لمتابعة حركة الأصناف، تسجيل البضاعة التالفة، ومعرفة نواقص المخزون.")
    st.download_button(label="📥 تحميل نموذج الجرد", data="هذا مجرد ملف تجريبي لتعمل الأداة", file_name="inventory_template.txt")
    
    st.subheader("3. كشف مطابقة رصيد (Word)")
    st.write("نموذج احترافي جاهز للطباعة لمطابقة الأرصدة وإغلاق الحسابات مع الموردين وشركات الشحن.")
    st.download_button(label="📥 تحميل كشف المطابقة", data="هذا مجرد ملف تجريبي لتعمل الأداة", file_name="balance_statement.txt")
    
    st.info("💡 نصيحة: تقديم هذه القوالب مجاناً للزوار يجعلهم يشاركون رابط موقعك، مما يرفع عدد الزيارات بشكل طبيعي ومجاني.")
