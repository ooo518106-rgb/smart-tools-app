...
def export_to_pdf(title, rows, filename="report.pdf"):
    try:
        ...
        pdf.cell(0, 12, "Smart Merchant Tools", ln=True, align="C")
        pdf.set_font("Helvetica", "", 11)
        pdf.cell(0, 8, "Business Report", ln=True, align="C")
        pdf.ln(6)
        ...
        for label, value in rows:
            label_s = str(label).encode("latin-1", "ignore").decode("latin-1")
            value_s = str(value).encode("latin-1", "ignore").decode("latin-1")
            pdf.cell(90, 9, label_s, border=1)
            pdf.cell(0, 9, value_s, border=1, ln=True)

        pdf.ln(5)
        pdf.set_font("Helvetica", "I", 9)
        pdf.cell(0, 8, "Generated by Smart Merchant Tools", ln=True, align="C")
...
def pdf_download_button(title, rows, filename="report.pdf", key_suffix=""):
    pdf_bytes = export_to_pdf(title, rows, filename)
    if pdf_bytes:
        st.download_button(
            "📤 تحميل PDF",
            data=pdf_bytes,
